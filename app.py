...
# ====== Environment ======
TWELVE_KEY   = os.getenv("TWELVE_KEY")      # ضع المفتاح في Render → Environment
DEFAULT_LOGO = os.getenv("LOGO_URL", "")    # اختياري: رابط PNG شفاف

# ====== Helpers ======
def fetch_ohlc(symbol: str, interval: str, bars: int = 300) -> pd.DataFrame:
    """
    يجلب OHLC من TwelveData.
    أمثلة interval: 1h, 30min, 15min, 4h, 1day
    """
    if not TWELVE_KEY:
        raise HTTPException(status_code=500, detail="TWELVE_KEY is not configured")
...
def chart(
    ...
    # (2) الثيم والشموع — خلفية داكنة، بدون شبكة، ألوان احترافية
    ...
    style = mpf.make_mpf_style(
        ...
        marketcolors=mc,
        facecolor=("#0f1115" if theme == "dark" else "white"),
        figcolor=("#0f1115" if theme == "dark" else "white"),
        edgecolor="#90939a",
        gridstyle="",   # ⇐ إطفاء شبكة mplfinance
    )
    ...
    ax = axes[0]
    ax.grid(False)  # احتياطيًا: إطفاء شبكة matplotlib
    ...
    # (4) خط السعر اللحظي + طابع زمني
    last_ts = df.index[-1]
    last_close = float(df["Close"].iloc[-1])  # استخدام iloc لتجنب تحذير Pandas
    ...
    ax.set_title(f"{ttl}     {ts_str}",
                 fontsize=14,
                 color=("#f2f2f2" if theme == "dark" else "black"))
